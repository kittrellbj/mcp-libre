"""
Live-verification probe for update_cell_comment_live (new tool, Brian's
new-tools assignment priority #11 -- distinct from add_cell_comment_live's
upsert semantics: requires the comment to already exist, and is the
only cell-comment tool that can toggle IsVisible).

Adds a real comment via add_cell_comment_live, then exercises
update_cell_comment_live against the real running extension: partial
text-only update, visibility toggle, and requiring an existing comment
-- checking each result against list_cell_comments_live's own read
(not just success=true).

Usage: python update-cell-comment-probe-windows.py
...
def main():
    print("update_cell_comment_live live-verification probe")
    ...
    soffice_log = REPO_DIR / "update-cell-comment-probe-soffice.log"
    with open(soffice_log, "w") as log_file:
        subprocess.Popen(
            [str(SOFFICE_EXE), "--headless", f"--accept={accept}", "--norestore"],
            stdout=log_file, stderr=subprocess.STDOUT,
        )
    bootstrap_path = REPO_DIR / "update-cell-comment-probe-bootstrap.py"
    ...
    print("\n[4/5] Add a real comment via add_cell_comment_live")
    r = http_post("/tools/add_cell_comment_live", {"cell": "B2", "text": "Original comment"})
    if not r.get("success"):
        fail(f"add_cell_comment_live failed: {r}")

    print("\n[5/5] update_cell_comment_live checks against real data")

    r = http_post("/tools/update_cell_comment_live", {"cell": "B2", "text": "Revised comment"})
    check("text-only update succeeds", r.get("success") is True)
    check("reports only 'text' as updated", r.get("success") and r["result"]["updated"] == ["text"])
    r2 = http_post("/tools/list_cell_comments_live", {})
    check("list_cell_comments_live confirms the real comment text actually changed",
          r2.get("success") and any(c["cell"] == "B2" and c["text"] == "Revised comment" for c in r2["result"]["comments"]))

    r = http_post("/tools/update_cell_comment_live", {"cell": "B2", "visible": True})
    check("visibility-only update succeeds", r.get("success") is True)
    check("reports only 'visible' as updated", r.get("success") and r["result"]["updated"] == ["visible"])

    r = http_post("/tools/update_cell_comment_live", {"cell": "Z99", "text": "No comment here"})
    check("updating a cell with no existing comment reports a clean OBJECT_NOT_FOUND failure",
          r.get("success") is False and r.get("error", {}).get("code") == "OBJECT_NOT_FOUND")
    ...
    print("\nPASS: all update_cell_comment_live checks passed against real headless LibreOffice.")
    sys.exit(0)
