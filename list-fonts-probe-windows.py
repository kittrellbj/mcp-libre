"""
Live-verification probe for list_fonts_live (new tool, Brian's new-tools
assignment priority #8, not document-scoped -- font availability is a
LibreOffice installation property).

Checks the tool against the real running extension: any headless
LibreOffice installation ships a handful of Liberation/DejaVu-family
fonts, so this checks for real, known font names rather than just
success=true, plus structural checks (grouped by name, no duplicate
names, at least one multi-style font reports more than one style).

Usage: python list-fonts-probe-windows.py
...
def main():
    print("list_fonts_live live-verification probe")
    ...
    soffice_log = REPO_DIR / "list-fonts-probe-soffice.log"
    with open(soffice_log, "w") as log_file:
        subprocess.Popen(
            [str(SOFFICE_EXE), "--headless", f"--accept={accept}", "--norestore"],
            stdout=log_file, stderr=subprocess.STDOUT,
        )
    bootstrap_path = REPO_DIR / "list-fonts-probe-bootstrap.py"
    ...
    print("\n[4/4] list_fonts_live checks against the real installation")

    r = http_post("/tools/list_fonts_live", {})
    check("request succeeds with no document open and no arguments", r.get("success") is True)
    fonts = r["result"]["fonts"]
    check("count matches the real fonts list length", r["result"]["count"] == len(fonts))
    check("at least a handful of real fonts are installed (not an empty/fake list)", len(fonts) >= 3)

    names = [f["name"] for f in fonts]
    check("no duplicate font names (grouped, not one entry per style variant)",
          len(names) == len(set(names)))

    known_families = {"Liberation Sans", "Liberation Serif", "Liberation Mono", "DejaVu Sans", "DejaVu Serif"}
    check("at least one real, known bundled font family is present",
          bool(known_families & set(names)))

    multi_style = [f for f in fonts if len(f["styles"]) > 1]
    check("at least one font reports more than one real installed style (e.g. Regular + Bold)",
          len(multi_style) > 0)
    for f in fonts:
        if not (isinstance(f["styles"], list) and f["styles"] == sorted(f["styles"])):
            fail(f"styles for {f['name']!r} are not sorted: {f['styles']}")
    print("  [PASS] every font's styles list is sorted")
    ...
    print("\nPASS: all list_fonts_live checks passed against real headless LibreOffice.")
    sys.exit(0)
